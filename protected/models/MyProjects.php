 ...
 */
class MyProjects extends CActiveRecord
{
	/**
	 * Returns the static model of the specified AR class.
	 * @param string $className active record class name.
	 * @return MyProjects the static model class
	 */
	...
	public function rules()
	{
		...
		return array(
			...
			array('userId, endday, create_user_id, update_user_id, bid, ProjectBudget', 'numerical', 'integerOnly'=>true),
			array('projectName', 'length', 'max'=>100),
			array('status', 'length', 'max'=>1),
			array('Description, startday, create_time, update_time', 'safe'),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('projectId, projectName, status, userId, Description, startday, endday, create_time, create_user_id, update_time, update_user_id, bid, ProjectBudget', 'safe', 'on'=>'search'),
		);
	}
